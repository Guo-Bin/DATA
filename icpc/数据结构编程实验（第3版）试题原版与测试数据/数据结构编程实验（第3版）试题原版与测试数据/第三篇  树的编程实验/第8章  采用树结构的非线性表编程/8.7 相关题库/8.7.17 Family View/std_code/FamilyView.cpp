...
namespace ACM{
    const int N = int(1e6) + 9, Z = 26, L = int(1e6) + 9;
    int trans[N][Z], fail[N], cnt[N], match[N], Q[N], cz, op, tt;
    char str[L];
...
#define f trans[fail[u]][c]
    ...
    void Build(){
        ...
        while (cz < op){
            u = Q[cz++]; REP(c, Z){
                if (v) fail[Q[op++] = v] = f, checkMax(cnt[v], cnt[f]);// .. .
                else v = f;
            }
        }
    }
...
#undef c
    void Run(){
        getchar(); gets(str); int res = 0; int t, u = 0; RST(vis);
